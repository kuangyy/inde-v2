...
//初始化区域图的对象
    ...
    if (property.haveHead) {
        ...
        for (var x = 0; x < property.headBtns.length; ++x) {
            tmp += "<a href='javascript:void(0)' class='GooFlow_head_btn'>"
                //自定义图标  type 用于指定事件
                + "<i class=\"fa "+ property.headBtns[x].icon +"\" type='"+ property.headBtns[x].type +"' aria-hidden=\"true\"></i>"
                + "</a>"
        }
        tmp += "</div>";
        this.$head = $(tmp);
        ...
        this.onBtnNewClick = function () { //新建
            var that = this;
            var title = prompt(ch('输入新的名称：'), 'newFlow_' + this.$defaultTitleId++);
            if (title != null) {
                this.clearData();
                this.setTitle(title);
            }
    ...
    if (property.haveTool) {
        ...
        //未加代码：加入绘图工具按钮 箭头 连线
        this.$tool.append("<a href='javascript:void(0)' type='cursor' class='GooFlow_tool_btndown' id='" + this.$id + "_btn_cursor'>"
            + "<i class=\"fa fa-mouse-pointer\" aria-hidden=\"true\"></i></a>"
            + "<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='" + this.$id + "_btn_direct'>"
        ...
        if (property.toolBtns && property.toolBtns.length > 0) {
            ...
            for (var i = 0; i < property.toolBtns.length; ++i) {
                if(!property.toolBtns[i].icon)property.toolBtns[i].icon = "fa-cog";
                tmp += "<a href='javascript:void(0)' "
                        //自定义名称
                    + "type='" + property.toolBtns[i].name
                    + "' id='" + this.$id + "_btn_" + property.toolBtns[i].name + "' class='GooFlow_tool_btn'>"
                        //自定义图标
                    + "<i class=\"fa "+ property.toolBtns[i].icon +"\" aria-hidden=\"true\"></i>"
                    + "</a>";//加入自定义按钮
            }
...
GooFlow.prototype = {
    ...
    addNode: function (id, json) {
        ...
        if (json.type.indexOf(" round") < 0) {
            ...
            var icon = "fa-empire";
            for( t in property.toolBtns){
                var _that = property.toolBtns[t];
                if(_that.name == json.type)icon=_that.icon;
            }

            this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr><td class='ico'>"
                + "<i class=\"fa "+ icon +"\" aria-hidden=\"true\"></i>"
                + "</td><td class='span'>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
